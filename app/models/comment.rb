class Comment < ApplicationRecord
  ...
  validates :body, length: { minimum: 1, maximum: YiffyAPI.config.comment_max_size }
  ...
  def update_last_commented_at_on_create
    ...
    if Comment.where("post_id = ?", post_id).count <= YiffyAPI.config.comment_threshold && !do_not_bump_post?
