class PostReplacement < ApplicationRecord
  ...
  def user_is_not_limited
    ...
    if post.replacements.where(creator_id: creator.id).where('created_at > ?', 1.day.ago).count >= YiffyAPI.config.post_replacement_per_day_limit
      self.errors.add(:creator, 'has already suggested too many replacements for this post today')
      throw :abort
    end
    if post.replacements.where(creator_id: creator.id).count >= YiffyAPI.config.post_replacement_per_post_limit
  ...
  module StorageMethods
    def remove_files
      ...
      YiffyAPI.config.storage_manager.delete_replacement(self)
    end
    ...
    def write_storage_file
      ...
      YiffyAPI.config.storage_manager.store_replacement(replacement_file, self, :original)
      thumbnail_file = PostThumbnailer.generate_thumbnail(replacement_file, is_video? ? :video : :image)
      YiffyAPI.config.storage_manager.store_replacement(thumbnail_file, self, :preview)
    ensure
      thumbnail_file.try(:close!)
    end

    def replacement_file_path
      YiffyAPI.config.storage_manager.replacement_path(self, file_ext, :original)
    end

    def replacement_thumb_path
      YiffyAPI.config.storage_manager.replacement_path(self, file_ext, :preview)
    end

    def replacement_file_url
      YiffyAPI.config.storage_manager.replacement_url(self)
    end

    def replacement_thumb_url
      YiffyAPI.config.storage_manager.replacement_url(self, :preview)
    end
  end
    ...
    def new_upload_params
      {
          ...
          file: YiffyAPI.config.storage_manager.open(YiffyAPI.config.storage_manager.replacement_path(self, file_ext, :original)),
