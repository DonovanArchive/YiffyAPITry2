  ...
  class Replacer
    ...
    def create_backup_replacement
      begin
        ...
        repl.replacement_file = YiffyAPI.config.storage_manager.open(YiffyAPI.config.storage_manager.file_path(post, post.file_ext, :original))
        repl.save
      rescue Exception => e
    ...
    def process!(penalize_current_uploader:)
      ...
      PostReplacement.transaction do
        replacement.replacement_file = YiffyAPI.config.storage_manager.open(YiffyAPI.config.storage_manager.replacement_path(replacement, replacement.file_ext, :original))
