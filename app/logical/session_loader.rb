class SessionLoader
  ...
  def initialize(request)
    ...
    @remember_validator = ActiveSupport::MessageVerifier.new(YiffyAPI.config.remember_key, serializer: JSON, digest: "SHA256")
  end
  ...
  def load
    ...
    CurrentUser.user.unban! if CurrentUser.user.ban_expired? && !YiffyAPI.config.readonly_mode?
    ...
    update_last_logged_in_at unless YiffyAPI.config.readonly_mode?
    update_last_ip_addr unless YiffyAPI.config.readonly_mode?
    set_time_zone
    set_safe_mode
  ...
  def set_safe_mode
    safe_mode = YiffyAPI.config.safe_mode? || params[:safe_mode].to_s.truthy? || CurrentUser.user.enable_safe_mode?
